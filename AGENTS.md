...
| Loại | Kiểm bằng gì |
|---|---|
| **Logic điều khiển** (khoá/mở, đếm giờ, thứ tự 6 mục) | Unit test — đúng/sai là **hành vi**, không cần thiết bị |
...
| 2 | **Màn hình gỡ lỗi** — camera + nhận diện + hiện số liệu thô | ✅ **XONG** |
| — | *(phát sinh)* **Bộ chạy thử bằng video** — xem mục dưới | ✅ **XONG** |
| 3 | Tầng đo đạc — **MỘT hàm duy nhất** dùng chung ảnh mẫu + camera | ✅ **XONG** |
| 4 | Engine hướng dẫn — 6 mục, khoá/mở, sinh câu chỉ dẫn | ✅ **XONG phần LOGIC** (04/09/2026) — ngưỡng còn tạm, chờ buổi đo |
...
| `log/` | `SessionLogger.kt` | **Ghi mọi số đo ra CSV** |
| `home/` | `HomeScreen.kt` | Lưới chọn ảnh mẫu + hộp thoại cổng kiểm |
| | `VideoPickerScreen.kt` | Chọn video đóng vai camera — **CHỈ có ở bản chạy thử**, gỡ khi dùng camera thật |
...
| `result/` | `ResultScreen.kt` + UiState/ViewModel | **Màn xem lại**: bày 5 ảnh cạnh ảnh mẫu + **bảng phân tích 6 mục** + **cảnh báo khi không tấm nào đạt**, giữ 1, xoá phần còn lại |
| `sim/` | `SimCameraScreen.kt` + UiState/ViewModel | Màn camera, video đóng vai camera |
| `debug/` | `DebugScreen.kt` + UiState/Action/ViewModel · `CameraController.kt` | Màn đo số liệu, dùng camera THẬT |
...
## Bộ chạy thử bằng video (không cần điện thoại)

Máy ảo không có người thật để soi, nên có đường thay thế: **video đóng vai camera**.

**Luồng màn hình hiện tại:**
```
MỘT bản build, HAI đường. Có **hai lối vào camera thật**: công tắc ở màn hình chính,
HOẶC nút "📷 Dùng camera thật" ngay trên màn chọn video (vì trên điện thoại, bước chọn
video là vô nghĩa — không có nút này là kẹt).

Ảnh mẫu lấy từ **thư viện soạn sẵn HOẶC người dùng tự chọn từ máy** (nút "+ Chọn ảnh từ
máy của bạn", dùng Photo Picker nên KHÔNG cần xin quyền đọc bộ nhớ). Ảnh tự chọn được
**chép vào `templates/`** — không giữ `Uri` gốc, vì quyền đọc nó hết hạn khi app tắt.
Cả hai loại ảnh đi qua **đúng một cổng kiểm**, không có đường tắt.

  [Camera thật]  Chọn ảnh mẫu → CỔNG KIỂM → MÀN CHỤP ────────────→ MÀN XEM LẠI
                                            quay .mp4 Full HD       5 ảnh khớp nhất
                                            → chấm điểm từng khung  giữ 1, xoá hết
                                            → cắt lại ảnh nét cao
                                            → XOÁ video thô

  [Video giả lập] Chọn ảnh mẫu → CỔNG KIỂM → Chọn cảnh quay → Màn camera giả lập → MÀN XEM LẠI
                                 🔴 chặn hẳn  (chỉ bản thử)    khung xương + CSV
                                 🟡 cảnh báo
                                 🟢 vào thẳng
```
Bước "Chọn cảnh quay" **chỉ tồn tại ở bản chạy thử** — bản thật là camera mở lên. Tách riêng
thành `VideoPickerScreen.kt` để sau gỡ ra gọn.

```
test-media/1-camera-gia-lap/   → video .mp4 đứng thay camera
test-media/2-anh-mau/          → ảnh mẫu để chọn
```
...
### Rút nhật ký đo về phân tích

```bash
adb pull /sdcard/Android/data/com.example.posecoach/files/logs/
```

Mỗi khung hình một dòng: `time_ms, detected, points, yaw_deg, framing, inference_ms, event`.
Cột `event` đánh dấu `MAT_DAU` / `BAT_LAI` / `GIU_KHUNG`.

**File này chính là "bảng số đo" mà buổi đo trên máy thật yêu cầu** (§10.3) — không phải công
cụ dùng một lần rồi bỏ.

### Đã kiểm chứng bằng số liệu thật

| Phát hiện | Ý nghĩa |
|---|---|
| Góc xoay **0-1°** khi người nhìn thẳng vào máy | Xác nhận công thức đã sửa đúng (FOOTGUNS 13) |
| **Lớp khung hình nhảy KNEE → FULL** dù người đứng yên | **Bằng chứng thực tế** cho luật "suy lớp MỘT LẦN từ ảnh mẫu, không để live tự chọn" |
| Khung xương chồng khít lên người, 33/33 điểm | Lớp bọc nhận diện chạy đúng |
| 🎯 **Mẫu xoay đủ vòng (−171° → +176°): 280/280 khung vẫn thấy người, KHÔNG mất dấu lần nào** | **RỦI RO SỐ 1 ĐÃ GIẢI** — nhóm ảnh mẫu quay lưng giữ nguyên trong scope. Vẫn giữ luồng `ACQUIRING` làm lưới an toàn |
| Ảnh `faceless` (mặt bị mũ che) vẫn nhận diện được | Tài liệu Google viết *"the **head** should be visible"* — cái ĐẦU, không phải khuôn mặt |

⚠️ **Tốc độ khung hình và độ nóng máy đo trên máy ảo KHÔNG dùng được** — vẫn phải đo trên
điện thoại thật.
...
### 4. Nhóm CHẤT LƯỢNG của iOS có 6 thành phần, ta mới có 2
...
| **`eyesOpen` (mắt nhắm)** | 0,06 | ❌ **thiếu thật** — không thuộc diện đã cắt scope |

⚠️ `eyesOpen` đáng chú ý nhất: ảnh mẫu chớp mắt là ảnh hỏng, **không liên quan gì tới ánh sáng
hay rung tay** nên không nằm trong phần PO đã cắt. Cần ML Kit Face Detection → **phải hỏi PO
trước khi thêm thư viện**.

### 5. Luật cách nhau giữa 5 ảnh của ta lỏng hơn

iOS đòi **0,9 giây** VÀ **khác dáng ≥ 0,12**; ta chỉ đòi **0,4 giây**. Đã thấy hậu quả thật
trên máy ảo: 5 tấm trả về gần như giống hệt nhau.
...
- Dáng ngồi/nằm (chỉ hỗ trợ dáng ĐỨNG, kể cả tựa tường)
...
- **Danh sách 6 điều kiện có dấu tích phải SỐNG** — bám trạng thái `CriterionGate` thật, không
